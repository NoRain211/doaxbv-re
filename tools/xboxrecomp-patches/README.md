...
The pinned fork revision already contains this fix, so apply the patch only to
an upstream checkout that lacks it, before regenerating C. It assumes the
...
`recomp-runtime/recomp_types.h`. The pinned fork already includes it; apply it
only to an upstream checkout with the FSUBP patch, then run from that checkout:
...
`test_lifter_casino_flags` compiles and runs synthetic sequences, so it needs
gcc or clang:
