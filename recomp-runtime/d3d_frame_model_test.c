...
int recomp_d3d_frame_model_test(void)
{
    ...
    result = recomp_d3d_frame_clear(
        &state, 0u, 0u, RECOMP_D3D_CLEAR_KNOWN_FLAGS,
        0u, 0x3f800000u, 0u);
    passed &= expect_u32(
    ...
    result = recomp_d3d_frame_clear(
        &state, 1u, 0x12345678u, RECOMP_D3D_CLEAR_KNOWN_FLAGS,
        0u, 0x3f800000u, 0u);
    passed &= expect_u32(
        "clear rectangles",
        result.error,
    ...
    result = recomp_d3d_frame_clear(
        &state, 0u, 0x12345678u, RECOMP_D3D_CLEAR_KNOWN_FLAGS,
        0u, 0x3f800000u, 0u);
    passed &= expect_u32(
    ...
    passed &= expect_u32(
        ...
        1u);
    result = recomp_d3d_frame_clear(
        &state, 0u, 0u, RECOMP_D3D_CLEAR_TARGET,
        0u, 0x7fc00000u, 0x100u);
    passed &= expect_u32(
        "color-only clear", result.error, RECOMP_D3D_FRAME_OK);
    passed &= expect_u32(
        "color-only color", result.command.data.clear.clear_color, 1u);
    passed &= expect_u32(
        "color-only depth", result.command.data.clear.clear_depth, 0u);
    passed &= expect_u32(
        "color-only stencil", result.command.data.clear.clear_stencil, 0u);
    result = recomp_d3d_frame_clear(
        &state, 0u, 0u, RECOMP_D3D_CLEAR_DEPTH,
        0u, 0x3f800000u, 0x100u);
    passed &= expect_u32(
        "depth-only clear", result.error, RECOMP_D3D_FRAME_OK);
    passed &= expect_u32(
        "depth-only color", result.command.data.clear.clear_color, 0u);
    passed &= expect_u32(
        "depth-only depth", result.command.data.clear.clear_depth, 1u);
    passed &= expect_u32(
        "depth-only stencil", result.command.data.clear.clear_stencil, 0u);
    result = recomp_d3d_frame_clear(
        &state, 0u, 0u, RECOMP_D3D_CLEAR_STENCIL,
        0u, 0x7fc00000u, 0xffu);
    passed &= expect_u32(
        "stencil-only clear", result.error, RECOMP_D3D_FRAME_OK);
    passed &= expect_u32(
        "stencil-only color", result.command.data.clear.clear_color, 0u);
    passed &= expect_u32(
        "stencil-only depth", result.command.data.clear.clear_depth, 0u);
    passed &= expect_u32(
        "stencil-only stencil", result.command.data.clear.clear_stencil, 1u);
    result = recomp_d3d_frame_clear(
        &state, 0u, 0u, 0u, 0u, 0x7fc00000u, 0x100u);
    passed &= expect_u32(
        "empty clear", result.error, RECOMP_D3D_FRAME_OK);
    passed &= expect_u32(
        "empty clear color", result.command.data.clear.clear_color, 0u);
    passed &= expect_u32(
        "empty clear depth", result.command.data.clear.clear_depth, 0u);
    passed &= expect_u32(
        "empty clear stencil", result.command.data.clear.clear_stencil, 0u);
    result = recomp_d3d_frame_clear(
